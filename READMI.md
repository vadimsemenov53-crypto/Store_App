...
- get_avg_price_products - метод вычисления средней цены товаров.
...
В проекте реализована обработка ошибок при работе с товарами, категориями и заказами.

### 🔹 Проверка количества товара
При создании товара выполняется проверка количества.
Если пользователь пытается создать товар с нулевым количеством, выбрасывается исключение ValueError.
```python
if quantity == 0:
    raise ValueError("Товар с нулевым количеством не может быть добавлен.")
```
Это предотвращает добавление некорректных товаров в систему.

### 🔹 Пользовательское исключение
В проекте создано собственное исключение:
```python
...
Метод add_product() обрабатывает попытку добавить товар с нулевым количеством.
...
Логика работы:
- если количество товара равно 0 — вызывается исключение
- если ошибок нет — товар добавляется в категорию
- блок finally гарантирует выполнение завершающего сообщения

### 🔹 Создание заказа

В классе Order также реализована проверка количества товара.
Если пользователь пытается оформить заказ с нулевым количеством, вызывается исключение ZeroProductQuantity.
...
Проект использует современные инструменты Python-разработки:
- Python 3
- pytest
- pytest-cov
- mypy
